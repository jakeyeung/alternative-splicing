...
tomtom_path <- 'G:/jyeung/projects/ALTERN~1/output/MISO_O~1/RUBIN_~1/SEHG19~1.GFF/T_TEST~1/FILTER~2/MOTIF_~1/MIAD77~1/fasta/summary/tomtom.summary.forplot.txt'
...
fname='G:/jyeung/projects/alternative_splicing/figs/tomtom_results_human_arial.rerun.eps'
...
g <- ggplot(tomtom_data, aes(x=as.numeric(region), y=pval_log, label=gene), legend=FALSE) +
    # add symbols
    geom_point(aes(shape=25, fill=motif_id, size=60)) +
    scale_fill_discrete(guide=FALSE) +
    # label symbosl with text
    geom_text(aes(x=as.numeric(region)+text_offset, y=text_pval_log, color=motif_id, size=60), hjust=0) +
